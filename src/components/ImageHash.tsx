...
import { Box, Divider, Grid, LinearProgress, Paper, Table, TableBody, TableCell, TableHead, TableRow, Typography } from "@suid/material"
...
export const ConfigView: Component<{ config: commons.config.Config, imageHash: string }> = (props) => {
  const signersOf = () => {
    const res = universal.genericCoderFor(props.config.version).config.signersOf(props.config)
  ...
  return <>
    ...
    <Grid container spacing={2}>
      <Flag grid label="Image Hash" value={props.imageHash} />
      ...
      <Grid item width="100%">
        <Paper>
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            <TableBody>
              <For each={signersOf()}>{(s) =>
                <TableRow>
                  <TableCell component="th" scope="row">{s.name}</TableCell>
                  <TableCell align="right"><Typography variant="code">{s.address}</Typography></TableCell>
    ...
    <Show when={v2.config.isWalletConfig(props.config)}>
      <ConfigViewV2 config={props.config as v2.config.WalletConfig} imageHash={props.imageHash} />
    </Show>
    <Show when={v1.config.ConfigCoder.isWalletConfig(props.config)}>
      <ConfigViewV1 config={props.config as v1.config.WalletConfig} imageHash={props.imageHash} />
    </Show>
  </>
}

export const ConfigViewV2: Component<{ config: v2.config.WalletConfig, imageHash: string }> = (props) => {
  const isComplete = () => {
    const res = v2.config.ConfigCoder.isComplete(props.config)
    return { value: res ? 'Yes' : 'No', status: res ? 'green' : 'red' as FlagStatus}
  }
  return <>
    <h3>Wallet config v2</h3>
    <Grid container spacing={2}>
      <Flag grid label="Threshold" value={props.config.threshold.toString()} />
      <Flag grid label="Checkpoint" value={props.config.checkpoint.toString()} />
...
export const ConfigV2NodeView: Component<{ node: v2.config.Topology, background?: number }> = (props) => {
  ...
  return <Box sx={{ borderRadius: '0.25rem' }} backgroundColor={`rgb(${back()}, ${back()}, ${back()})`} p="0.5rem">
    ...
    <Box ml='0.15rem' mb='0.25rem' mr='0.15rem' mt='0.05rem'>
      ...
      <Typography marginLeft='0.5rem' variant="code">{v2.config.hashNode(props.node)}</Typography>
    </Box>
    <Show when={open()}>
...
export const ConfigViewV1: Component<{ config: v1.config.WalletConfig, imageHash: string }> = (props) => {
  return <Grid container spacing={2} mt="1rem">
    <Flag grid label="Threshold" value={props.config.threshold.toString()} />
  </Grid>
}

export const ImageHashView: Component<{ imageHash: string }> = (props) => {
  if (!mayBeImageHash(props.imageHash)) {
    return <></>
  }
  ...
  createEffect(() => {
    setLoading(true)
    setConfig(undefined)
    TRACKER.configOfImageHash(props).then((c) => {
      setConfig(c)
      setLoading(false)
    }).catch((e) => {
      console.error(e)
  ...
  return <>
    ...
    <Show when={config()} keyed>
      {(config) => <ConfigView config={config} imageHash={props.imageHash} /> }
    </Show>
    <Show when={!config() && !loading()}>
      <div>Imagehash info not found</div>
    </Show>
  </>
}